...
def depth_experiment():
    depth = [1.5, 2.5, 3.5, 4.5, 5.5, 6.5]
    name = ['1p5', '2p5', '3p5', '4p5', '5p5', '6p5']
    for i in range(len(depth)):
        experiment(depth[i], name[i])


def experiment(depth, name, seed=1):
    print("Sensitivity Experiment (PointSGRADE)! with depth {}".format(name))
    np.random.seed(seed)
    num_experiments = 30
    ...
    for i in range(num_experiments):
        ...
        """
        options = AnomalyParas(per_num_points=150, depth=depth, radius=1.75, num_anomaly=5, num_knot_b_spline=12, random_state=np.random.randint(0, 10000), noise_std_z=0.06, noise_std_xy=0.025, step=0.5)
        ...
        np.save('./result/sensitivity_depth/data/pts_' + str(i) + '_k_' + name, pts)
        np.save('./result/sensitivity_depth/data/label_' + str(i) + '_k_' + name, label)
        ...
        np.save('./result/sensitivity_depth/result/estimated label_' + str(i) + '_k_' + name, estimated_label)
