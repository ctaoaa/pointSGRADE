...
def analysis(num_expr=30, radius=None, root='./result/sensitivity_radius'):
    accuracies = np.zeros((len(radius), num_expr))
    dices, for_, fpr_, running_time = [accuracies.copy() for _ in range(4)]
    for i in range(len(radius)):
        for j in range(num_expr):
            label = np.load(root + '/data/' + 'label_' + str(j) + '_k_' + radius[i] + '.npy')
            estimated_label = np.load(root + '/result/' + 'estimated label_' + str(j) + '_k_' + radius[i] + '.npy')
...
def depth_FOR(data):
    ...
    plt.xlabel('Radius ' + r'$r$', fontdict={'family': 'Times New Roman', 'size': 26})
    ...
    x_list = [1.0, 1.3, 1.6, 1.9, 2.2, 2.5]
    ...
    plt.xlim(0.9, 2.6)
    plt.xticks([1.0, 1.3, 1.6, 1.9, 2.2, 2.5])
    plt.savefig("./result/figures/sensitivity_radius_FOR.jpg", dpi=300)
    plt.show()
...
def depth_FPR(data):
    ...
    plt.xlim(0.9, 2.6)
    plt.xticks([1.0, 1.3, 1.6, 1.9, 2.2, 2.5])
    plt.savefig("./result/figures/sensitivity_radius_FPR.jpg", dpi=300)
    plt.show()
...
def depth_BA(data):
    ...
    plt.xlim(0.9, 2.6)
    plt.xticks([1.0, 1.3, 1.6, 1.9, 2.2, 2.5])
    plt.savefig("./result/figures/sensitivity_radius_BA.jpg", dpi=300)
    plt.show()
...
def depth_DICE(data):
    ...
    plt.xlim(0.9, 2.6)
    plt.xticks([1.0, 1.3, 1.6, 1.9, 2.2, 2.5])
    plt.savefig("./result/figures/sensitivity_radius_DICE.jpg", dpi=300)
    plt.show()


if __name__ == "__main__":
    Radius = ['1p0', '1p3', '1p6', '1p9', '2p2', '2p5']
